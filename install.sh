...
# Usage: curl -fsSL https://git.brznet.fr/brz/adtui/raw/branch/main/install.sh | sh
...
REPO_URL="https://git.brznet.fr/brz/adtui"
VERSION="latest"
...
detect_install_method() {
    ...
    if command -v pip &> /dev/null; then
        INSTALL_METHOD="pip"
        log_info "Found pip, using Python package installation"
    elif command -v python3 &> /dev/null; then
        INSTALL_METHOD="source"
        log_info "Found Python3, installing from source"
    else
        INSTALL_METHOD="binary"
        log_info "No Python found, downloading binary executable"
    fi
}
...
install_pip() {
    ...
    if pip show adtui &> /dev/null; then
        ...
        pip install git+https://git.brznet.fr/brz/adtui.git
    fi

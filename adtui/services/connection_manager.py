...
try:
    from .config_service import ADConfig
except ImportError:
    from services.config_service import ADConfig

# Logging is configured centrally in logging_service.setup_logging().
...
class ConnectionManager:
    ...
    def __init__(
        ...
        self._state_change_callbacks: list[
            Callable[[ConnectionState, Optional[str]], None]
        ] = []
    ...
    def add_state_change_callback(
        self, callback: Callable[[ConnectionState, Optional[str]], None]
    ):
    ...
    def _create_connection(self) -> Connection:
        ...
        logger.info(
            f"Creating connection to {self.ad_config.server}:{port} as {bind_dn}"
        )
        ...
        if not self.ad_config.use_ssl:
            logger.warning(
                "Connected without SSL. Password operations will be disabled."
            )
    ...
    def _connect(self):
        ...
        try:
            ...
            if self._is_authentication_error(error_msg):
                logger.error(
                    f"Authentication error detected during initial connect - not retrying: {e}"
                )
    ...
    def _schedule_reconnect(self):
        ...
        delay = min(
            self.initial_retry_delay * (2**self._retry_count), self.max_retry_delay
        )
        ...
        logger.info(
            f"Scheduling reconnection in {delay:.1f} seconds (attempt {self._retry_count})"
        )
    ...
    def _reconnect(self):
        ...
        try:
            ...
            if self._is_authentication_error(error_msg):
                logger.error(
                    f"Authentication error detected during reconnect - not retrying: {e}"
                )
                self._trigger_auth_failure()
                return

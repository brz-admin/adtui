"""Details pane widget for displaying AD object information."""
...
class DetailsPane(Static):
    ...
    def action_set_password(self):
        ...
        if self.current_type == "user":
            ...
            self.app.push_screen(
                SetPasswordDialog(self.current_dn, self.current_connection_manager)
            )
        else:
            self.app.notify(
                "Password setting only available for users", severity="warning"
            )
    ...
    def action_manage_groups(self):
        ...
        if self.current_type == "user":
            ...
            self.app.push_screen(
                ManageGroupsDialog(
                    self.current_dn,
                    self.current_connection_manager,
                    self.user_details,
                    self.app.base_dn,
                )
            )
            ...
            self.app.push_screen(
                ManageGroupMembersDialog(
                    self.current_dn, self.current_connection_manager, self.group_details
                )
            )
        else:
            self.app.notify(
                "Group management not supported for this object type",
                severity="warning",
            )
    ...
    def action_view_attributes(self):
        ...
        self.app.push_screen(
            EditAttributesDialog(self.current_dn, self.current_connection_manager)
        )
        ...
        try:
            ...
            if entries:
                ...
                os_name = (
                    ...
                    if hasattr(entry, "operatingSystem")
                    else "N/A"
                )
                os_version = (
                    str(entry.operatingSystemVersion.value)
                    if hasattr(entry, "operatingSystemVersion")
                    else "N/A"
                )
                dns_hostname = (
...
"""
    ...
    def refresh_details(self):
        ...
        if self.current_type == "user" and self.user_details:
            ...
            self._show_ou_details(
                self.current_label, self.current_dn, self.current_connection_manager
            )

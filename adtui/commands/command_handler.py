...
class CommandHandler:
    ...
    def execute(self, command_str: str) -> None:
        ...
        if command_str.startswith('/'):
            ...
            if query:
                ...
                self.app.notify(MESSAGES['SEARCH_EMPTY'], severity=Severity.WARNING)
            return
        ...
        if handler:
            ...
            self.app.notify(f"Unknown command: {command}", severity=Severity.WARNING)
    ...
    def _handle_search(self, query: str) -> None:
        ...
        try:
            ...
            self.app.notify(f"Error searching AD: {e}", severity=Severity.ERROR)
    
    def _handle_delete(self, args: str) -> None:
        """Handle delete command."""
        if not self.app.current_selected_dn:
            self.app.notify(MESSAGES['NO_SELECTION'], severity=Severity.WARNING)
            return
    ...
    def _handle_move(self, target_path: str) -> None:
        ...
        if not target_path:
            self.app.notify(MESSAGES['TARGET_REQUIRED'], severity=Severity.WARNING)
            return
        ...
        if not self.app.ldap_service.validate_ou_exists(target_dn):
            self.app.notify(MESSAGES['TARGET_OU_NOT_FOUND'].format(dn=target_dn), severity=Severity.ERROR)
            return
    ...
    def _handle_create_ou(self, path: str) -> None:
        ...
        if not path:
            self.app.notify(MESSAGES['OU_PATH_REQUIRED'], severity=Severity.WARNING)
            return
    ...
    def _handle_recycle(self, args: str) -> None:
        ...
        try:
            ...
            self.app.notify(str(e), severity=Severity.ERROR)
    
    def _handle_restore(self, cn: str) -> None:
        """Handle restore command."""
        if not cn:
            self.app.notify(MESSAGES['RESTORE_NAME_REQUIRED'], severity=Severity.WARNING)
            return
        ...
        try:
            ...
            if result is None:
                self.app.notify(MESSAGES['NO_MATCH'].format(query=cn), severity=Severity.WARNING)
            elif 'error' in result and result['error'] == 'multiple':
                self.app.notify(MESSAGES['MULTIPLE_MATCHES'].format(query=cn), severity=Severity.WARNING)
            else:
    ...
    def _handle_undo(self, args: str) -> None:
        ...
        if not self.app.history_service.can_undo():
            self.app.notify(MESSAGES['NO_UNDO_HISTORY'], severity=Severity.INFORMATION)
            return
        
        last_op = self.app.history_service.get_last()
        
        if last_op.type == 'delete':
            self.app.notify(MESSAGES['UNDO_DELETE_WARNING'], severity=Severity.WARNING)
            ...
            self.app.notify(f"Cannot undo operation type: {last_op.type}", severity=Severity.WARNING)
